# dsh-systray 真实 UI 截图脚本（简单直接：屏幕截取 client 内容区 → WebP）
# 用法: powershell -ExecutionPolicy Bypass -File capture_shots.ps1
# 原理: 通过 DSH_SYSTRAY_SHOT_PAGE 让前端启动后直接显示指定页，逐页重启进程；
#       截取窗口 client 内容区（不含标题栏/阴影），四周内缩裁剪圆角露底，
#       输出全尺寸 PNG 后由 convert_webp.py 高质量缩放转 WebP。
