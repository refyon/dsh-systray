...
func (f *Frontend) setupChromium() {
	...
	// [dsh-systray patch] 截图模式：DSH_SYSTRAY_CDP_PORT 非空时开启 WebView2 CDP 调试端口，
	// 供 capture_cdp.py 经 Page.captureScreenshot 直出渲染截图（高保真）。
	if cdpPort := os.Getenv("DSH_SYSTRAY_CDP_PORT"); cdpPort != "" {
		chromium.AdditionalBrowserArgs = append(chromium.AdditionalBrowserArgs, "--remote-debugging-port="+cdpPort)
		chromium.AdditionalBrowserArgs = append(chromium.AdditionalBrowserArgs, "--remote-allow-origins=*")
	}
